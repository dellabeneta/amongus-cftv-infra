let startReactor = {
    ...
    scores: {}, // Armazenar as pontuações dos jogadores
    ...
    play(index) {
        ...
        if (startReactor.isTheRightCombination(startReactor.playerCombination.length)) {
            if (startReactor.playerCombination.length == startReactor.combinationMaxPosition) {
                ...
                startReactor.updateScore("player1"); // Atualizar pontuação
                setTimeout(() => {
                    startReactor.start();
                }, 1200);
                return;
    ...
    updateScore(playerName) {
        if (!startReactor.scores[playerName]) {
            startReactor.scores[playerName] = 0;
        }
        startReactor.scores[playerName]++;
        startReactor.showTopPlayers(); // Mostrar os melhores jogadores
    },

    showTopPlayers() {
        const sortedPlayers = Object.entries(startReactor.scores)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 10);

        const topPlayersDiv = document.getElementById("topPlayers");
        topPlayersDiv.innerHTML = "<h3>Top 10 Jogadores</h3>";
...
// Chame a função load no início para inicializar os eventos e carregar os sons
startReactor.load().then(() => startReactor.start());
